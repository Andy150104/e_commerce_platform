<template>
  <div class="w-full relative">
    <swiper-container
      class="centered-slide-carousel relative"
      pagination
      space-between="50"
      :slides-per-view="1"
    >
      <swiper-slide v-for="(image, index) in imageList" :key="index">
        <div
          class="bg-indigo-50 rounded-2xl flex justify-center items-center lg: h-[38rem]"
          :class="{'h-96': !isMobile, 'h-56': isMobile}"
        >
          <img
            class="w-full h-full rounded-lg object-cover lg: max-w-4xl"
            :src="image"
            alt=""
          />
        </div>
