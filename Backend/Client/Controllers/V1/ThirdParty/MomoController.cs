...
namespace Client.Controllers.V1.ThirdParty
{
    ...
    public class MomoController : ControllerBase
    {
        ...
        public async Task<IActionResult> PaymentOrderCallBack()
        {
            ...
            var response = _momoService.PaymentExecuteOrderAsync(HttpContext.Request.Query);

            // Get order
            var order = _orderService.Find(or => or.OrderId == Guid.Parse(response.OrderId)).FirstOrDefault()!;

            var user = _userService.Find(u => u.UserName == response.FullName,
                    true,
                    u => u.Addresses)
                .FirstOrDefault();
            if (user == null)
            ...
            var extraValues = response.ExtraData.Split('|');
            // PlatForm
            ...
            MomoOrderSendMail.SendMailGhnCode(_emailTemplateService, ghnCode, user.Email, _appDbContext, detailErrorList);

            if (platForm.Equals(((byte)ConstantEnum.Platform.Web).ToString()))
            {
                if (response.ErrorCode == "0")
                    return Redirect($"{CommonUrl.WebUrlTrackingSuccess}{order.OrderId}");
                else
