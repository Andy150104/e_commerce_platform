...
public class OrderService : BaseService<Order, Guid, VwOrder>, IOrderService
{
    ...
    public OrderService(IBaseRepository<Order, Guid, VwOrder> repository, IMomoService momoService,
        IAddressService addressService, IAccessoryService accessoryService, IGHNLogic ghnLogic) : base(repository)
    {
    ...
    public InsertOrderResponse InsertOrder(InsertOrderRequest request, IIdentityService identityService)
    {
        ...
        Repository.ExecuteInTransaction(() =>
        {
            ...
            {
                var accessory = _accessoryService.Find(a => a.AccessoryId == item.AccessoryId && a.IsActive == true)
                    .FirstOrDefault();
                if (accessory == null)
                {
